...
//! 見出しの下に必ず空ける余白（本文の文字の高さに対する割合）
/*!
	🔥 **これが 0 だと見出しが次の行にくっついて「詰まって見える」**（2026-09-05 本人から指摘）。
	   サクラは全行が同じ高さの升目なので、**見出しの行だけ下を空けることはできない**。
	   ∴ 「いちばん大きい見出し＋この余白」が升目の高さになるよう逆算する
	   ＝見出しの下に必ずこのぶんの隙間が残る。

	   ⚠ 代償として `.md` の**本文どうしの行間も同じだけ広がる**（升目が全部そろって高くなるため）。
	     見出しの下だけ空ける手（見出しを上へずらす等）は、
	     文字が升目からはみ出して**前の行を消す・描き直しで欠ける**ので採らない。
*/
constexpr double MD_HEADING_GAP_SCALE = 0.30;

//! いちばん大きい見出しが収まるように、行の高さをこの倍率まで広げる
/*!
	🔥 **いちばん大きい見出しの倍率より必ず大きくしておく。**
	   行の高さが足りないと、見出しはそこで頭打ちになって大きくならない
	   （上下が欠けないよう `CalcHeadingHeight()` が切り詰めるため）。
	   ⚠ ここを上げると `.md` の**全部の行**の間隔が広がる。これがこの仕掛けの代償。

	   ＝「いちばん大きい見出し＋その下の余白」。数字を直に書かないこと
	   （直に書くと、見出しを大きくしたときに余白が黙って消える）。
*/
constexpr double MD_LINE_HEIGHT_SCALE = MD_HEADING_SCALE[0] + MD_HEADING_GAP_SCALE;
